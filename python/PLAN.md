...
> the `python/` package and the agreed next steps, because the original planning
...
(`osfstorage-archive/make_figures_behavsim.m`) — as a clean, testable Python
...
- Dataset location is overridable with `THINGS_ARCHIVE_DIR`; cache location with
  `THINGS_CACHE_DIR` (see `paths.py`).
...
- [ ] Short README for `python/` (install, `THINGS_*` env vars, how to run on the
